...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 13:12:49 2020
...
class COVID19_Dataset(data.Dataset):
    """
    ...
    def __init__(self, 
        ...
        np.random.seed(seed)  # Reset the seed so all runs are the same.
        csvpath=os.path.join(thispath, folder+"_split_v2.csv")
        imgpath=os.path.join(thispath, "data", folder)
        self.imgpath = imgpath
        self.transform = transform
        self.data_aug = data_aug
       # self.views = views
        ...
        self.pathologies = ["COVID-19","normal","pneumonia"]
        self.pathologies = sorted(self.pathologies)
        ...
        self.labels = []
        for pathology in self.pathologies:
            mask = self.csv['label'].str.contains(pathology)
            if pathology in mapping:
                for syn in mapping[pathology]:
                    #print("mapping", syn)
                    mask |= self.csv['label'].str.contains(syn)
            self.labels.append(mask.values)
        self.labels = np.asarray(self.labels).T
        self.labels = self.labels.astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        X_im =Image.open(img_path).convert('RGB')
        img = self.transform(X_im)
        ...
        # print(img.shape)
        # return {"PA":img, "lab":torch.tensor(np.argmax(self.labels[idx])), "idx":idx}
        return [img, np.argmax(self.labels[idx])]#img, self.labels[idx]#, idx
